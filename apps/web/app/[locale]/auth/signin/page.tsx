...
// Force dynamic if needed, although it's a client component
...
// ─── Social Providers (Refactored to avoid anonymous component creation) ──────
...
const SOCIAL_PROVIDERS = [
  { id: 'google' as const, label: 'Google', disabled: false, icon: GoogleIcon },
  { id: 'facebook' as const, label: 'Facebook', disabled: false, icon: FacebookIcon },
  { id: 'apple' as const, label: 'Apple', disabled: true, icon: AppleIcon },
];
...
// ─── Sign In Content (Stabilized Hooks) ──────────────────────────────────────
...
function SignInContent() {
  ...
  // Sync mode with URL only once or when searchParams change safely
  useEffect(() => {
  ...
  const handleSocialLogin = useCallback(async (providerId: string) => {
    ...
    try {
      ...
      const redirectUrl = `${origin}/api/auth/callback?next=${encodeURIComponent(`/${locale}/parent`)}`;
  ...
  // Render Role Buttons safely
  const roleButtons = useMemo(() => ROLES.map((r) => {
    const Icon = r.icon;
  ...
  // Render Social Buttons safely
  const socialButtons = useMemo(() => SOCIAL_PROVIDERS.map((provider) => (
    <button
      key={provider.id}
      type="button"
      ...
      >
            ...
            <form onSubmit={handleEmailAuth} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="mt-1 relative">
                    ...
                    <input
                      ...
                      className="block w-full rounded-xl border border-gray-200 bg-white py-3 pl-10 pr-4 text-gray-900 focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20 transition-all"
              ...
              <div>
                ...
                <div className="mt-1 relative">
                  ...
                  <input
                    ...
                    className="block w-full rounded-xl border border-gray-200 bg-white py-3 pl-10 pr-12 text-gray-900 focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20 transition-all"
                    placeholder="••••••••"
              ...
              {isLogin && (
                <div className="flex items-center justify-end">
                  <Link href={`/${locale}/auth/forgot-password`} className="text-sm font-medium text-violet-600 hover:text-violet-500 transition-colors">
              ...
              <button
                type="submit"
                disabled={loading}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 py-3 px-4 text-sm font-semibold text-white shadow-lg shadow-violet-500/30 transition-all hover:scale-[1.01] disabled:opacity-50"
              >
                {loading ? (
        ...
        <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-violet-600 via-indigo-600 to-purple-700 items-center justify-center p-12 relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-lg text-center">
            ...
            <h2 className="text-3xl font-bold text-white mb-4">{t('quote')}</h2>
            <p className="text-lg text-white/80">{t('quoteSubtext')}</p>
          </div>
        </div>
      </div>
    </>

...
const ADMIN_EMAIL = "admin@freegeny.com";
...
async function main() {
  ...
  if (existing.rows.length > 0) {
    await pool.query(
      `UPDATE users SET password_hash = $1, full_name = $2, role = 'parent', onboarding_step = 4, updated_at = NOW() WHERE email = $3`,
      [hash, ADMIN_NAME, ADMIN_EMAIL]
    );
    console.log("✅ Admin mis à jour:", ADMIN_EMAIL);
  } else {
    await pool.query(
      `INSERT INTO users (email, username, password_hash, full_name, role, onboarding_step, created_at, updated_at)
       VALUES ($1, $2, $3, $4, 'parent', 4, NOW(), NOW())`,
      [ADMIN_EMAIL, "freegeny_admin", hash, ADMIN_NAME]
    );

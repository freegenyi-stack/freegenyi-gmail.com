...
// Mon Atelier — bootstrap éditeur (page edit + iframe interne H5P)
window.ns = window.H5PEditor = window.H5PEditor || {};
if (!window.H5PEditor.language) {
  window.H5PEditor.language = {};
}

var usingParentIntegration = false;
try {
  if (
    window.parent &&
    window.parent !== window &&
    window.parent.H5PIntegration &&
    window.parent.H5PIntegration.editor &&
    window.parent.H5PIntegration.editor.ajaxPath
  ) {
    window.H5PIntegration = window.parent.H5PIntegration;
    usingParentIntegration = true;
  }
} catch (e) {
  /* cross-origin */
}

if (typeof window.H5PIntegration !== "object" || window.H5PIntegration === null) {
  window.H5PIntegration = {};
}
H5PIntegration = window.H5PIntegration;

if (!usingParentIntegration) {
  H5PIntegration.loadedJs = H5PIntegration.loadedJs || [];
  H5PIntegration.loadedCss = H5PIntegration.loadedCss || [];
  H5PIntegration.loadedJs.length = 0;
  H5PIntegration.loadedCss.length = 0;
}
if (!H5PIntegration.editor) {
  H5PIntegration.editor = { enableContentHub: false };
}
if (!H5PIntegration.l10n) {
  H5PIntegration.l10n = { H5P: {} };
}

if (H5PIntegration.editor) {
  ns.ajaxPath = ns.ajaxPath || H5PIntegration.editor.ajaxPath;
  ns.assets = ns.assets || H5PIntegration.editor.assets;
  ns.contentLanguage = ns.contentLanguage || H5PIntegration.editor.language;
  ns.basePath = ns.basePath || H5PIntegration.editor.libraryUrl;
}

ns.getAjaxUrl = function (action, parameters) {
  var base = ns.ajaxPath || (H5PIntegration.editor && H5PIntegration.editor.ajaxPath) || "/ix/ajax?action=";
  var url = base + action;
      ...
      ns.$.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown) {
          if (window.console !== undefined) {
            console.warn("H5P editor: library ajax failed", libraryName, textStatus, errorThrown);
          }
          ns.libraryCache[libraryName] = { semantics: [] };
          var failCallbacks = function () {
            callback(null);
            if (ns.loadedCallbacks[libraryName]) {
              for (var i = 0; i < ns.loadedCallbacks[libraryName].length; i++) {
                ns.loadedCallbacks[libraryName][i](null);
              }
              ns.loadedCallbacks[libraryName] = [];
            }
          };

...
import { School, MapPin, Users, Globe } from "lucide-react";
...
export default async function SchoolDashboardPage({
  ...
  const labels = {
    ...
    approvedDesc: t("approvedDescSchool"),
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-16">
          <div>
            <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight font-jakarta uppercase">
              FreeGeny <span className="text-indigo-600">Écoles</span>
            </h1>
            <p className="text-slate-500 font-medium mt-2 text-lg italic">{t("schoolSubtitle")}</p>
          </div>
          <div className="bg-white border-2 border-indigo-100 p-4 rounded-2xl shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{t("establishment")}</p>
              <p className="text-sm font-black text-slate-900">{metadata.institutionSchoolName || user.fullName}</p>
            </div>
          </div>
        </div>
        ...
        {!showFullDashboard ? (
          <OrgVerificationDashboard
            ...
            productName={getOrgProductName("ecole")}
            labels={labels}
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 bg-white rounded-[3rem] p-12 shadow-xl border border-slate-50">
              <h2 className="text-2xl font-black text-slate-900 mb-6 uppercase tracking-tighter">{t("institutionalDashboard")}</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100">
                  <Users className="w-8 h-8 text-indigo-600 mb-4" />
                  <h3 className="text-lg font-black text-slate-900 mb-1">{t("connectedStudents")}</h3>
                  <p className="text-3xl font-black text-indigo-600">0</p>
                </div>
                <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100">
                  <Globe className="w-8 h-8 text-indigo-600 mb-4" />
                  <h3 className="text-lg font-black text-slate-900 mb-1">{t("freeService")}</h3>
            ...
            <div className="space-y-8">
              <div className="bg-white rounded-[2.5rem] p-8 shadow-lg border border-slate-50">
                <h3 className="text-sm font-black uppercase text-slate-900 mb-6 flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-indigo-600" /> {t("location")}
                </h3>
                <p className="text-xs text-slate-500 font-bold leading-relaxed">
                  {metadata.institutionAddress || t("addressMissing")}
                </p>
              </div>
              <div className="bg-indigo-600 rounded-[2.5rem] p-8 text-white shadow-lg shadow-indigo-200">
                ...
                <p className="text-2xl font-black mb-2">FreeGeny Écoles</p>
              </div>
            </div>
          </div>
        )}

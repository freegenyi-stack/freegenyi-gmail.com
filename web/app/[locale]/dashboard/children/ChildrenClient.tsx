...
import ChildProfileEditSheet from "@/components/children/ChildProfileEditSheet";
    ...
    <div className="space-y-12" dir={isRTL ? "rtl" : "ltr"}>
      ...
      >
        ...
        <div className={cn("relative z-10", isRTL && "text-right")}>
          <h1 className={cn("text-4xl md:text-5xl font-black text-slate-900 tracking-tight", isRTL && "font-ui-ar")}>
            {t("title")}
          </h1>
      ...
      {isParent && (
        <div className={cn("flex justify-end", isRTL && "justify-start")}>
          <button
            onClick={() => setIsModalOpen(true)}
            className={cn(
              "flex items-center gap-3 rounded-2xl bg-orange-500 px-6 py-3 text-[11px] font-black uppercase tracking-widest text-white shadow-lg shadow-orange-500/25 transition hover:bg-orange-400",
              isRTL && "flex-row-reverse font-ui-ar text-sm tracking-normal"
            )}
          >
            <Plus className="w-4 h-4" />
        ...
        <AnimatePresence mode="popLayout">
          {children.map((child, index) => (
            <motion.div
              ...
              className={cn(
                "bg-white rounded-[2.5rem] p-8 shadow-sm border relative group transition-all duration-500 overflow-hidden",
                isParent
                  ? "border-orange-100 hover:border-orange-200 hover:shadow-md"
                  : "shadow-[0_32px_80px_rgba(0,0,0,0.03)] border-slate-50 hover:shadow-[0_40px_100px_rgba(0,0,0,0.06)]"
              )}
            >
              ...
              <div className={cn("flex items-center gap-6 mb-8", isRTL && "flex-row-reverse")}>
                ...
                <div className={isRTL ? "text-right" : ""}>
                  <h3 className={cn("text-xl font-black text-slate-900 tracking-tight", isRTL && "font-ui-ar")}>
                    {child.fullName}
                  </h3>
                  <div className={cn("flex items-center gap-2 mt-1", isRTL && "flex-row-reverse")}>
                    <span className="w-2 h-2 bg-green-500 rounded-full" />
                    <span className={cn("text-[10px] font-black uppercase tracking-widest text-slate-400", isRTL && "font-ui-ar normal-case text-xs")}>
                      {t("activeProfile")}
                    </span>
                  </div>
                </div>
              ...
              <div className="space-y-4 mb-8">
                <div className={cn("flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100", isRTL && "flex-row-reverse")}>
                  <div className={cn("flex items-center gap-3 text-slate-500", isRTL && "flex-row-reverse")}>
                    ...
                    <span className={cn("text-[11px] font-black uppercase tracking-wider", isRTL && "font-ui-ar normal-case")}>
                      {t("level")}
                    </span>
                  </div>
              ...
              <button
                ...
                className={cn(
                  "w-full bg-white border-2 border-slate-100 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-700 hover:border-orange-400 hover:text-orange-700 transition-all",
                  isRTL && "font-ui-ar text-sm normal-case"
                )}
              >
                {t("editProfile")}
              </button>
            ...
            >
              <div className="p-10 md:p-12">
                <div className={cn("flex justify-between items-start mb-10", isRTL && "flex-row-reverse")}>
                  <div className={isRTL ? "text-right" : ""}>
                    <h2 className={cn("text-3xl font-black text-slate-900 tracking-tight", isRTL && "font-ui-ar")}>
                      {t("modalTitle")}
                    </h2>
                  ...
                  <div className="space-y-2">
                    <label
                      className={cn(
                        ...
                        isRTL && "font-ui-ar normal-case flex-row-reverse"
                      )}
                    >
      ...
      <AnimatePresence>
        {editChild && (
          <ChildProfileEditSheet
            ...
            initialProfileJson={editChild.learningProfile}
            locale={locale}

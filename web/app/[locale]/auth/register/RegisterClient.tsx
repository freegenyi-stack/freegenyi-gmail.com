...
const COUNTRIES = [
    ...
    { name: 'Mexico / México', code: 'MX', flag: '🇲🇽', dial: '+52' },
...
export default function RegisterClient({ locale }: { locale: string }) {
    ...
    const [childCountry, setChildCountry] = useState(regionCountry || (isNorwegian ? "NO" : (isFinnish || isFinnishSwedish ? "FI" : (isDutch ? "NL" : (isPortuguese ? "PT" : (isSpanish ? "AR" : (isPolish ? "PL" : (isCzech ? "CZ" : (isJapanese ? "JP" : (regionCountry === "BR" ? "BR" : (regionCountry === "CL" ? "CL" : (regionCountry === "CO" ? "CO" : (regionCountry === "MX" ? "MX" : (regionCountry === "SG" ? "SG" : "DZ"))))))))))))));

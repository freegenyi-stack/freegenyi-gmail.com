...
export default function LoginClient({ locale }: { locale: string }) {
  ...
  const isSpanish = selectedLang === "es" || locale === "es" || locale.endsWith("-es");
                     ...
                     <div className="space-y-4">
                         <div className="space-y-1.5">
                             <label className={`block text-[11px] font-black uppercase tracking-wider text-slate-700 px-1 ${isArabic ? "text-right" : "text-left"}`}>
                                {isMaori ? "Īmēra" : isIrish ? "Ríomhphost" : isNorwegian ? "E-post" : isSwedish ? "E-post" : isDanish ? "E-mail" : isFinnish || isFinnishSwedish ? "Sähköposti" : isDutch ? "E-mail" : isPortuguese ? "E-mail" : isSpanish ? "Correo electrónico" : isEnglish ? "E-mail" : isArabic ? "البريد الإلكتروني" : "E-mail"}
                             </label>
                             <div className="relative">
                                 ...
                                 <input 
                                     ...
                                     name="email" 
                                     required 
                                    placeholder={isMaori ? "hemi@example.co.nz" : isIrish ? "aoife@eire.ie" : isNorwegian ? "ole@example.no" : isSwedish ? "erik@example.se" : isDanish ? "mads@example.dk" : isSpanish ? "correo@ejemplo.com" : isEnglish ? "email@example.com" : isArabic ? "mail@example.com" : "nom@exemple.com"} 
                         ...
                         <div className="space-y-1.5">
                             <div className={`flex justify-between px-1 ${isArabic ? "flex-row-reverse" : ""}`}>
                                 <label className="block text-[11px] font-black uppercase tracking-wider text-slate-700">
                                    {isMaori ? "Kupuhipa" : isIrish ? "Focal Faire" : isNorwegian ? "Passord" : isSwedish ? "Lösenord" : isDanish ? "Adgangskode" : isFinnish || isFinnishSwedish ? "Salasana" : isDutch ? "Wachtwoord" : isPortuguese ? "Palavra-passe" : isSpanish ? "Contraseña" : isPolish ? "Hasło" : isCzech ? "Heslo" : isChinese ? "密码" : isKorean ? "비밀번호" : isJapanese ? "パスワード" : isMalay ? "Kata laluan" : isTamil ? "கடவுச்சொல்" : isEnglish ? "Password" : isArabic ? "كلمة المرور" : "Mot de passe"}
                                 </label>
                                 <Link href="/auth/forgot" className="text-[10px] font-bold text-orange-600 hover:underline uppercase tracking-wider">
                                    {isMaori ? "Kua wareware?" : isIrish ? "Dearmad déanta?" : isNorwegian ? "Glemt?" : isSwedish ? "Glömt?" : isDanish ? "Glemt?" : isFinnish || isFinnishSwedish ? "Unohdut?" : isDutch ? "Wachtwoord vergeten?" : isPortuguese ? "Esqueceu a palavra-passe?" : isSpanish ? "¿Olvidaste?" : isPolish ? "Zapomniałeś?" : isCzech ? "Zapomněli jste?" : isChinese ? "忘记了？" : isKorean ? "비밀번호 찾기" : isJapanese ? "お忘れですか？" : isMalay ? "Lupa?" : isTamil ? "மறந்துவிட்டீர்களா?" : isEnglish ? "Forgot?" : isArabic ? "نسيت كلمة المرور؟" : "Oublié ?"}
                                 </Link>
                             </div>
                             <div className="relative">
                     ...
                     <div className="flex flex-col sm:flex-row items-center gap-4 pt-3">
                         ...
                         >
                             ...
                             <span className="text-[10px] font-black uppercase tracking-wider group-hover:text-orange-600">
                                {isMaori ? "Takiuru Google" : isIrish ? "Logáil isteach Google" : isNorwegian ? "Google Logg inn" : isSwedish ? "Google Logga in" : isDanish ? "Google Log ind" : isFinnish || isFinnishSwedish ? "Google Kirjaudu sisään" : isDutch ? "Google Inloggen" : isPortuguese ? "Entrar com Google" : isSpanish ? "Entrar con Google" : isPolish ? "Zaloguj przez Google" : isCzech ? "Přihlásit se přes Google" : isChinese ? "使用 Google 登录" : isKorean ? "Google 로그인" : isJapanese ? "Googleでログイン" : isMalay ? "Log masuk dengan Google" : isTamil ? "Google மூலம் உள்நுழைக" : isEnglish ? "Google Sign In" : isArabic ? "اتصال بجوجل" : "Connexion Google"}
                             </span>
                         </button>
                         ...
                         >
                             {isSubmitting 
                                ? (isMaori ? "E hono ana..." : isIrish ? "Ag ceangal..." : isNorwegian ? "Kobler til..." : isSwedish ? "Ansluter..." : isDanish ? "Forbinder..." : isFinnish || isFinnishSwedish ? "Yhdistetään..." : isDutch ? "Verbinden..." : isPortuguese ? "A ligar..." : isSpanish ? "Conectando..." : isPolish ? "Łączenie..." : isCzech ? "Připojování..." : isChinese ? "连接中..." : isKorean ? "연결 중..." : isJapanese ? "接続中..." : isMalay ? "Menyambung..." : isTamil ? "இணைக்கிறது..." : isEnglish ? "Connecting..." : isArabic ? "جاري الاتصال..." : "Accès...") 
                                : (isMaori ? "Takiuru" : isIrish ? "Logáil Isteach" : isNorwegian ? "Logg inn" : isSwedish ? "Logga in" : isDanish ? "Log ind" : isFinnish || isFinnishSwedish ? "Kirjaudu sisään" : isDutch ? "Inloggen" : isPortuguese ? "Entrar" : isSpanish ? "Iniciar sesión" : isPolish ? "Zaloguj się" : isCzech ? "Přihlásit se" : isChinese ? "登录" : isKorean ? "로그인" : isJapanese ? "ログイン" : isMalay ? "Log Masuk" : isTamil ? "உள்நுழை" : isEnglish ? "Sign In" : isArabic ? "تسجيل الدخول" : "Se connecter")}

/** Algeria wilayas — code → FR / AR labels for SchoolPicker & seeds */
const DZ_WILAYAS = [
...
const byCode = new Map(DZ_WILAYAS.map((w) => [w.code, w]));
const byFrNorm = new Map(
  DZ_WILAYAS.map((w) => [w.fr.toLowerCase().normalize("NFD").replace(/\p{Diacritic}/gu, ""), w])
);
...
function formatDzWilayaName(code, fallbackFr, lang) {
  const c = String(code || "").padStart(2, "0");
  const row = byCode.get(c) || byFrNorm.get(normFr(fallbackFr));
  if (!row) return fallbackFr || "";
  return lang === "ar" ? row.ar : row.fr;
}

function formatDzCommuneName(name, lang) {
  if (!name) return "";
  const paren = name.match(/\(([^)]+)\)/);
  const arabic = name.replace(/\s*\([^)]+\)\s*/g, "").trim();
  if (lang === "ar") return arabic || name;
  if (paren) return paren[1].trim();
  return name;
}

function dzAdminLabels(lang) {
  return lang === "ar"
    ? { region: "الولاية", commune: "البلدية" }
    : { region: "Wilaya", commune: "Commune" };
}

...
export default function SchoolPicker({
  ...
  const getRegionName = (r: Region) => isNorwegian || isFinnish ? (r.nameLocal || r.nameFr) : isSwedish ? (r.nameLocal || r.nameFr) : isEnglish ? (r.nameLocal || r.nameFr) : isAr ? (r.nameLocal || r.nameFr) : (r.nameFr || r.nameLocal);
  const getDistrictName = (d: District) => isNorwegian || isFinnish ? (d.nameLocal || d.nameFr) : isSwedish ? (d.nameLocal || d.nameFr) : isEnglish ? (d.nameLocal || d.nameFr) : isAr ? (d.nameLocal || d.nameFr) : (d.nameFr || d.nameLocal);
  const getSchoolName = (s: SchoolResult) => isNorwegian || isFinnish ? (s.nameLocal || s.nameFr) : isSwedish ? (s.nameLocal || s.nameFr) : isEnglish ? (s.nameLocal || s.nameFr) : isAr ? (s.nameLocal || s.nameFr) : (s.nameFr || s.nameLocal);
  const getSchoolRegion = (s: SchoolResult) => isNorwegian || isFinnish ? (s.regionNameLocal || s.regionNameFr) : isSwedish ? (s.regionNameLocal || s.regionNameFr) : isEnglish ? (s.regionNameLocal || s.regionNameFr) : isAr ? (s.regionNameLocal || s.regionNameFr) : (s.regionNameFr || s.regionNameLocal);
  const getSchoolDistrict = (s: SchoolResult) => isNorwegian || isFinnish ? (s.districtNameLocal || s.districtNameFr) : isSwedish ? (s.districtNameLocal || s.districtNameFr) : isEnglish ? (s.districtNameLocal || s.districtNameFr) : isAr ? (s.districtNameLocal || s.districtNameFr) : (s.districtNameFr || s.districtNameLocal);
      ...
      <div className="flex items-center gap-3 bg-emerald-50 border-2 border-emerald-200 rounded-2xl px-5 py-3" dir={isAr ? "rtl" : "ltr"}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-0.5">
            {isAr ? "المدرسة المحددة" : isMaori ? "Kura Kua Tohua" : isIrish ? "Scoil Roghnaithe" : isNorwegian ? "Valgt skole" : isSwedish ? "Vald skola" : isDanish ? "Valgt skole" : isFinnish ? "Valittu koulu" : isEnglish ? "Selected School" : "École sélectionnée"}
          </p>
          ...
          <div className="relative flex-1">
            ...
            >
              <span className="flex items-center gap-2 truncate">
                ...
                <span className="truncate">{selectedDistrict ? getDistrictName(selectedDistrict) : (isAr ? "البلدية" : country === "NO" ? "Kommune" : country === "FI" ? "Kunta" : "Commune")}</span>
              </span>
              <div className="flex items-center gap-1 shrink-0 ml-1">
                {districtCode && (
                  <div 
            ...
            >
              <span className="flex items-center gap-2 truncate">
                ...
                <span className="truncate">
                  {selectedRegion
                    ? getRegionName(selectedRegion)
                    : (isAr ? "الولاية" : country === "IE" ? (isIrish ? "Contae" : "County") : country === "GB" ? "Region" : country === "AU" ? "State" : country === "US" ? "State" : country === "NZ" ? (isMaori ? "Rohe" : "Regional Council") : country === "DK" ? "Region" : country === "SE" ? "Län" : country === "NO" ? "Fylke" : country === "FI" ? "Maakunta" : "Wilaya")}
                </span>
              </span>
              <div className="flex items-center gap-1 shrink-0 ml-1">
                {regionCode && (
            ...
            >
              <span className="flex items-center gap-2 truncate">
                ...
                <span className="truncate">
                  {selectedDistrict
                    ? getDistrictName(selectedDistrict)
                    : (isAr ? "البلدية" : country === "IE" ? (isIrish ? "Údarás Áitiúil" : "Local Authority") : country === "GB" ? "Local Authority" : country === "AU" ? "Suburb" : country === "US" ? "District" : country === "NZ" ? (isMaori ? "Takiwā" : "Territorial Authority") : country === "DK" ? "Kommune" : country === "SE" ? "Kommuner" : country === "NO" ? "Kommune" : country === "FI" ? "Kunta" : "Commune")}
                </span>
              </span>
              <div className="flex items-center gap-1 shrink-0 ml-1">
                {districtCode && (
        ...
        <div className="space-y-2 mt-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder={isAr ? "ابحث عن المدرسة..." : isNorwegian ? "Søk etter skole..." : isSwedish ? "Sök efter skola..." : isDanish ? "Søg efter skole..." : isFinnish ? "Etsi koulua..." : placeholder}
          ...
          <div className="flex gap-2">
            ...
            >
              {isAr ? "الكل" : isMaori ? "Katoa" : isIrish ? "Gach" : isNorwegian ? "Alle" : isSwedish ? "Alla" : isDanish ? "Alle" : isFinnish ? "Kaikki" : isEnglish ? "All" : "Toutes"}
            </button>
            <button
              type="button"
            ...
            >
              {isAr ? "عام" : isMaori ? "Tūmatanui" : isIrish ? "Poiblí" : isNorwegian ? "Offentlig" : isSwedish ? "Grundskola" : isDanish ? "Folkeskole" : isFinnish ? "Julkinen" : isEnglish ? "Public" : "Publiques"}
            </button>
            <button
              type="button"
            ...
            >
              {isAr ? "خاص" : isMaori ? "Tūmataiti" : isIrish ? "Príobháideach" : isNorwegian ? "Privat" : isSwedish ? "Privatskola / Friskola" : isDanish ? "Privatskole / Friskole" : isFinnish ? "Yksityinen" : isEnglish ? "Private" : "Privées"}
            </button>
          </div>
        </div>
      ) : (

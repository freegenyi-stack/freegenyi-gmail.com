...
const CSV_MX = path.join(__dirname, "data", "ecoles_primaires_mexique.csv");
...
async function seedMexicoSchools() {
  ...
  try {
    ...
    let countryRes = await client.query("SELECT id FROM countries WHERE code = 'MX'");
    if (countryRes.rows.length === 0) {
      await client.query(
        `INSERT INTO countries (code, name_fr, name_en, name_local, flag_emoji, langs, is_active)
         VALUES ('MX', 'Mexique', 'Mexico', 'México', '🇲🇽', 'es', true)`
      );
      console.log("🇲🇽 Mexico added to countries table.");
    } else {
      console.log("🇲🇽 Mexico already exists in countries table.");
    }

    console.log("📖 Loading ecoles_primaires_mexique.csv...");
    // Read with utf-8 and strip BOM
    const csvContent = fs.readFileSync(CSV_MX, "utf-8").replace(/^\uFEFF/, "");
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const departementName = values[idx("ESTADO")];
      const muniName = values[idx("MUNICIPIO")];
      const schoolName = values[idx("NOMBRE")];
      const schoolCodeRaw = values[idx("CCT")];
      const sector = values[idx("SECTOR")]; // PÚBLICO / PRIVADO
      ...
      if (!districtsMap.has(longKey)) {
        districtCode = `MX_D${districtIndex++}`;
      ...
      const isPrivate = (sector !== "PÚBLICO" && sector !== "PUBLICO");
    ...
    console.log(`📊 ${regionsMap.size} estados, ${districtsMap.size} municipios.`);

    console.log("📁 Seeding regions...");
    await client.query("DELETE FROM regions WHERE country_code = 'MX'");
    ...
    const dbRegions = await client.query("SELECT id, code FROM regions WHERE country_code = 'MX'");
    for (const r of dbRegions.rows) regionIds.set(r.code, r.id);

    console.log("📁 Seeding districts...");
    await client.query(`
      DELETE FROM districts
      WHERE region_id IN (SELECT id FROM regions WHERE country_code = 'MX')
    `);
    ...
    await client.query(`
      ...
      WHERE district_id IN (
        SELECT id FROM districts WHERE region_id IN (SELECT id FROM regions WHERE country_code = 'MX')
      )
    `);
    ...
    for (let i = 0; i < validSchools.length; i += batchSize) {
      ...
      for (const school of batch) {
        ...
        const base = row * 8;
        placeholders.push(
          `($${base + 1}, $${base + 2}, $${base + 3}, $${base + 4}, $${base + 5}, $${base + 6}, $${base + 7}, $${base + 8}, 'sep.gob.mx')`
        );
    ...
    `);
    console.log(`✅ Mexico seed complete: ${countRes.rows[0].count} schools in database.`);
  } catch (error) {
    console.error("❌ Error seeding Mexico schools:", error);
    process.exitCode = 1;
  } finally {
    await client.end();
  }

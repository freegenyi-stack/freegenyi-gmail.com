...
try {
    ...
    $conversations = DB::fetchAll("
        SELECT conv.*, 
               ...
               (SELECT last_login_at FROM users u 
                JOIN conversation_members m ON u.id = m.user_id 
                WHERE m.conversation_id = conv.id AND u.id != ? LIMIT 1) as other_user_last_login,
    ...
    if (isset($_GET['type']) && $_GET['type'] === 'contacts') {
        $family_members = DB::fetchAll("
            SELECT id, full_name, role, last_login_at as other_user_last_login, (last_login_at > DATE_SUB(NOW(), INTERVAL 3 MINUTE)) as is_online FROM users 
        ...
        foreach ($family_members as $mem) {
            ...
            if (!$existing) {
                $contacts[] = [
                    ...
                    'name' => $mem['full_name'],
                    'role' => $mem['role'],
                    'other_user_last_login' => $mem['other_user_last_login'],

...
<body class="min-h-[100dvh] w-full flex items-center justify-center p-4 sm:p-6 relative overflow-x-hidden overflow-y-auto">
    ...
    <div class="w-full max-w-7xl flex flex-col lg:flex-row items-center justify-center gap-16 relative z-10">
        ...
        <!-- Right : Formulaire -->
        <div class="w-full max-w-lg mt-6 lg:mt-0">
            <div class="glass-card rounded-[2.5rem] sm:rounded-[3.5rem] shadow-[0_45px_100px_rgba(0,0,0,0.07)] border border-white p-5 pt-10 sm:p-8 sm:pt-10 relative" x-data="{ role: 'parent' }">
                ...
                <div class="mb-3 mt-1 sm:mb-4 sm:mt-2 text-center">
                    <h1 class="text-xl sm:text-2xl font-black text-slate-950 font-title tracking-tight mb-0.5">Créer un compte.</h1>
                    <p class="text-slate-400 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest hidden sm:block">Cockpit d'exception pour futurs génies</p>
                </div>

                <!-- Sélecteur de rôle -->
                <div class="flex bg-slate-50 p-1 rounded-[14px] sm:rounded-2xl gap-1.5 sm:gap-2 mb-3 border border-slate-100">
                    <button @click="role = 'parent'" :class="role==='parent' ? 'bg-slate-950 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-200'" class="flex-1 py-1.5 sm:py-2.5 rounded-xl text-[10px] sm:text-[11px] font-black uppercase tracking-wider transition-all">Parent</button>
                    <button @click="role = 'school'" :class="role==='school' ? 'bg-orange-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-200'" class="flex-1 py-1.5 sm:py-2.5 rounded-xl text-[10px] sm:text-[11px] font-black uppercase tracking-wider transition-all">École</button>
                    <button @click="role = 'ngo'"    :class="role==='ngo'    ? 'bg-teal-600 text-white shadow-lg'   : 'text-slate-400 hover:bg-slate-200'" class="flex-1 py-1.5 sm:py-2.5 rounded-xl text-[10px] sm:text-[11px] font-black uppercase tracking-wider transition-all">ONG</button>
                </div>

                <!-- Bouton Google -->
                <a href="/api/auth/social.php?provider=google<?= !empty($_GET['invite_parent']) ? '&invite_parent=' . urlencode($_GET['invite_parent']) : '' ?>" class="flex items-center justify-center gap-3 w-full border-2 border-slate-100 hover:border-orange-400 bg-white hover:bg-orange-50 text-slate-700 font-bold py-2 sm:py-2.5 rounded-xl sm:rounded-2xl transition-all duration-200 shadow-sm mb-3 group">
                    <svg class="w-4 sm:w-5 h-4 sm:h-5" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                    <span class="text-[10px] sm:text-[11px] uppercase tracking-widest group-hover:text-orange-600 transition-colors">S'inscrire via Google</span>
                </a>
                ...
                <form action="/api/auth/register.php" method="POST" class="space-y-2">
                    ...
                    <div class="grid grid-cols-2 gap-2 sm:gap-3">
                        <div class="col-span-1">
                            ...
                            <input type="text" name="first_name" required placeholder="Ex: Jean" class="w-full bg-slate-50 border-2 border-slate-100 focus:border-orange-600 focus:bg-white px-3 py-2 sm:px-4 sm:py-3 rounded-xl outline-none transition-all font-semibold text-slate-950">
                        </div>
                        <div class="col-span-1">
                            <label class="block text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-slate-700 mb-0.5 sm:mb-1 px-1">Nom</label>
                            <input type="text" name="last_name" required placeholder="Ex: Martin" class="w-full bg-slate-50 border-2 border-slate-100 focus:border-orange-600 focus:bg-white px-3 py-2 sm:px-4 sm:py-3 rounded-xl outline-none transition-all font-semibold text-slate-950">
                        </div>
                        <div>
                            <label class="block text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-slate-700 mb-0.5 sm:mb-1 px-1">E-mail</label>
                            <input type="email" name="email" required placeholder="nom@exemple.com" class="w-full bg-slate-50 border-2 border-slate-100 focus:border-orange-600 focus:bg-white px-3 py-2 sm:px-4 sm:py-3 rounded-xl outline-none transition-all font-semibold text-slate-950">
                        </div>
                        <div>
                            <label class="block text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-slate-700 mb-0.5 sm:mb-1 px-1">Téléphone</label>
                            <input type="tel" name="phone" placeholder="+213..." class="w-full bg-slate-50 border-2 border-slate-100 focus:border-orange-600 focus:bg-white px-3 py-2 sm:px-4 sm:py-3 rounded-xl outline-none transition-all font-semibold text-slate-950">
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-slate-700 mb-0.5 sm:mb-1 px-1">Mot de passe</label>
                        <input type="password" name="password" required placeholder="••••••••" class="w-full bg-slate-50 border-2 border-slate-100 focus:border-orange-600 focus:bg-white px-3 py-2 sm:px-4 sm:py-3 rounded-xl outline-none transition-all font-semibold text-slate-950">
                    </div>
